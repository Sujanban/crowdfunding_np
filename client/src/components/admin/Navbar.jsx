...
const Navbar = () => {
    ...
    return (
        ...
        <>
                ...
                <ul className='pt-4 px-6 lg:px-10 grid gap-4 '>
                    {
                        nav.map((item, index) =>
                            <NavLink key={index} className={({ isActive }) => isActive ?
                                ...
                                {item.icon}
                                <span className={`${navbarVisible ? 'hidden transition-all duration-300' : 'px-2 block'}`}>{item.name}</span>
                            </NavLink>
                        )
                    }
                </ul>
            ...
            <div className={`md:hidden transition-all duration-300 w-screen ease-in-out h-full bg-slate-100 absolute z-[9999] left-0 top-[10vh]
            ${navbarVisible
                    ?
                    '-translate-x-full '
                    :
                    'translate-x-0'}
                 `}>
                <ul className={` pt-4 px-4 grid gap-4`}>
                    {
                        nav.map((item, index) =>
                            <NavLink onClick={hideSidebar} key={index} className={({ isActive }) => isActive ?
                                'p-2 rounded-xl bg-emerald-600 text-white text-md hover:bg-emerald-600 hover:text-white text-sate-700 flex items-center transition-all duration-300'
                                :
                                'p-2 rounded-xl  text-md hover:bg-emerald-600 hover:text-white text-slate-700 flex items-center  transition-all duration-300'} to={item.path}>
                                {item.icon}
                                <span className='px-2 block'>{item.name}</span>
                            </NavLink>
                        )
                    }
                    <NavLink onClick={hideSidebar} className={({ isActive }) => isActive ?
                        'p-2 space-x-1 rounded-xl bg-emerald-600 text-white text-md hover:bg-emerald-600 hover:text-white text-sate-700 flex items-center transition-all duration-300'
                        :
                        'p-2 space-x-1 rounded-xl  text-md hover:bg-emerald-600 hover:text-white text-slate-700 flex items-center  transition-all duration-300'} to='/profile'>
                        <GoPeople />
                        <span className='px-2 block'>Profile</span></NavLink>
                </ul>
            </div>
        </>
    )
