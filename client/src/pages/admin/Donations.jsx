        ...
        <div className='flex max-w-7xl mx-auto w-full rounded-xl'>
            ...
            <div className='w-full '>
                ...
                <div className='p-4 h-[90vh] overflow-y-auto bg-gray-100'>
                    ...
                    <div className='pt-2 md:pt-0 md:p-4  '>
                            ...
                            <div className='py-4'>
                                <div className='p-2 md:p-4 bg-white rounded-xl'>
                                    <div className='md:px-2 flex items-center justify-between'>
                                        <h1 className='text-sm md:text-md font-medium'>Recent Donations</h1>
                                        ...
                                        <table className="w-full text-sm text-left rtl:text-right text-gray-500 ">
                                            <thead className="text-xs text-slate-900 capitalize bg-gray-50">
                                                <tr className=''>
                                                    ...
                                                    <th scope="col" className="px-3 md:px-6 py-3"></th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                {
                                                    currentItems && currentItems.map((item) =>
                                                        <tr key={item._id} className='text-slate-600 text-xs sm:text-sm border-b'>
                                                            <td scope="col" className="px-3 md:px-6 py-2">{formatDate(item.createdAt)}</td>
                                                            <td scope="col" className="px-3 md:px-6 py-2 font-medium ">{item.userId?.email}</td>
                                                            <td scope="col" className="px-3 md:px-6 py-2">₹ {item.amount}</td>
                                                            <td scope="col" className="px-3 md:px-6 py-2">
                                                                <Link to={`/admin/donations/donation/${item._id}`} className='py-2 px-2 md:px-4 flex items-center text-slate-600  transition-all duration-300 hover:text-slate-900 rounded-xl'>View<HiOutlineExternalLink className='ml-2' size={15} /></Link>
                                                            </td>
                                                        </tr>
                                                    )
                                                }
