            ...
            <div className='p-4 grid gap-12 max-w-7xl mx-auto '>
                <div className='md:grid grid-cols-2 gap-4'>
                    <div className='md:p-4 m-auto'>
                        ...
                        <h1 className='py-2 md:pt-2 md:pb-1 font-medium'>Register Account</h1>
                        <p className=' text-slate-600 text-sm md:text-md'>Follow a simple steps by filling up email and password and create a account for fundraising.</p>
                    </div>
                    <img onClick={() => [setToggleFullImageView(true), setImage(screenshot1)]} className='w-[70%] cursor-pointer mx-auto p-1.5 rounded-xl bg-black' src={screenshot1} alt="" />
                </div>
                <div className='md:grid grid-cols-2 gap-4 flex flex-col-reverse'>
                    <img onClick={() => [setToggleFullImageView(true), setImage(screenshot2)]} className='w-[70%] cursor-pointer mx-auto p-1.5 rounded-xl bg-black' src={screenshot2} alt="" />
                    <div className='md:p-4 m-auto'>
                        <h1 className='w-12 h-12 md:w-16 md:h-16 flex items-center justify-center bg-yellow-500 rounded-full md:text-xl'>02</h1>
                        <h1 className='py-2 md:pt-2 md:pb-1 font-medium'>Create Campaign</h1>
                        <p className=' text-slate-600 text-sm md:text-md'>Enter your campaign details and start raising money.</p>
                    </div>
                </div>
                <div className='md:grid grid-cols-2 gap-4'>
                    <div className='md:p-4 m-auto'>
                        <h1 className='w-12 h-12 md:w-16 md:h-16 flex items-center justify-center bg-yellow-500 rounded-full md:text-xl'>03</h1>
                        <h1 className='py-2 md:pt-2 md:pb-1 font-medium'>Withdraw Money</h1>
                        <p className=' text-slate-600 text-sm md:text-md'>Add your bank details and Withdraw money from your account.</p>
                    </div>
                    <img onClick={() => [setToggleFullImageView(true), setImage(screenshot3)]} className='w-[70%] cursor-pointer mx-auto p-1.5 rounded-xl bg-black' src={screenshot3} alt="" />
                </div>
            </div>
            <Footer />
        </div>
