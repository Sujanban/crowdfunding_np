...
const Campaign3 = () => {
    const { useCampaigns, campaign, setCampaign } = useCampaign();
            ...
            <div className='lg:p-12 lg:grid grid-cols-3 gap-4'>
                <div className='col-span-1'>
                    ...
                    <div className='p-4 md:p-8 '>
                        ...
                        <h1 className='py-6 text-3xl md:text-4xl'>Add a cover photo or video</h1>
                        <p className='text-md'>Using a bright and clear photo helps people connect to your fundraiser right away.</p>

                    </div>
                </div>
                <div className='col-span-2 bg-white rounded-xl'>
                    <div className=' md:px-16 md:py-12 '>
                        <form className=' max-w-3xl  ' enctype="multipart/form-data">
                            <div className='p-2 flex justify-center '>
                                <label 
                                for="file-upload" 
                                className="m-4 w-full h-96 flex items-center justify-center border border-slate-600 border-dashed rounded-md cursor-pointer">
                                    {/* Custom Upload */}
                                    <MdOutlineCloudUpload size={30} />
                                </label>
                                <input
                                name='file' id="file-upload" type="file" onChange={(e) => setCampaign({ ...campaign, thumbnail: e.target.files[0] })} size="60" className='hidden' />
                            </div>
                            <p className='text-center'>or</p>
                            <div className=' w-full  p-4 block '>
                                <input
                                    onChange={(e) => setCampaign({ ...campaign, videoUrl: e.target.value })}
                                    className='w-full text-md font-light border border-stone-600 p-4 rounded-md'
                                    type="text"
                                    placeholder='Video URL' />
                            </div>
                            <div className='p-4 block md:flex justify-between '>
                                <Link to='/create/campaign/step2' className='w-full lg:w-auto px-5 py-3 bg-slate-800 text-white rounded-md'><MdArrowBack className='inline' /> Back</Link>
                                <Link to='/create/campaign/step4' className='w-full lg:w-auto px-5 py-3 bg-slate-800 text-white rounded-md'>Continue</Link>
                            </div>
                        </form>
                    </div>
                </div>
